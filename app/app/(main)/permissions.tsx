  ...
  return (
    ...
    >
      <View className="flex-row items-center mb-8 mt-12">
        ...
        <Text className="text-3xl font-bold text-vanguard-text-primary">Permissions</Text>
      </View>

      {permissions.length === 0 ? (
        <View className="items-center justify-center py-20">
          <Ionicons name="shield-outline" size={64} color={COLORS.primary} />
          <Text className="text-vanguard-text-primary font-bold text-xl mt-4">No Special Permissions</Text>
          <Text className="text-vanguard-text-secondary text-center mt-2">You haven't set any "Always Allow" rules yet.</Text>
        </View>
      ) : (
        permissions.map((permission) => (
          <View key={permission.id} className="bg-vanguard-card p-5 rounded-3xl mb-4 flex-row items-center border border-vanguard-border shadow-sm">
      ...
      <Text className="text-vanguard-text-secondary text-center text-xs mt-8 px-6">
        Permissions are automatically created when you select "Always Allow" on a request.
      </Text>
    </ScrollView>
  );
}
